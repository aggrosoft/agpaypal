...
<script src="https://www.paypal.com/sdk/js?components=buttons,messages,hosted-fields,funding-eligibility,payment-fields,marks[{if $oViewConf->getPayPalLocale()}]&locale=[{$oViewConf->getPayPalLocale()}][{/if}]&integration-date=2022-01-06&intent=capture[{if $oViewConf->getTopActiveClassName() === 'order'}]&enable-funding=[{$oView->getPayPalFunding()}][{elseif $oViewConf->getPayPalEnabledFundings()}]&enable-funding=[{$oViewConf->getPayPalEnabledFundings()}][{/if}]&commit=[{$paypalCommit}]&client-id=[{$oViewConf->getPayPalClientId()}]&currency=[{$currency->name}][{if $oViewConf->getTopActiveClassName() !== 'order' && $oViewConf->getPayPalDisabledFundings()}]&disable-funding=[{$oViewConf->getPayPalDisabledFundings()}][{/if}]"
